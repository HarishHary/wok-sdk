...
private:
    ...
    int post_think(C_CSPlayer* player) { // 56 8B 35 ? ? ? ? 57 8B F9 8B CE 8B 06 FF 90 ? ? ? ? 8B 07 (client_panorama.dll)
		...
		if (player->is_alive()) {
			...
			static const auto post_think_v_physics = reinterpret_cast<bool(__thiscall*)(C_CSPlayer*)>(SIG("client_panorama.dll", "55 8B EC 83 E4 F8 81 EC ? ? ? ? 53 8B D9 56 57 83 BB"));
			post_think_v_physics(player);
		}

		static const auto simulate_player_simulated_entities = reinterpret_cast<bool(__thiscall*)(C_CSPlayer*)>(SIG("client_panorama.dll", "56 8B F1 57 8B BE ? ? ? ? 83 EF 01 78 72"));
